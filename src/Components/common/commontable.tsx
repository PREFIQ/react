...
export interface TableRowData {
  /** every cell value can be a string OR a React node (e.g. buttons) */
  [key: string]: string | React.ReactNode;
  /** the special `id` field */
  id: string;
}
...
function CommonTable({
  ...
  return [...body].sort((a, b) => {
    const aRaw = a[sortColumn.toLowerCase()];
    const bRaw = b[sortColumn.toLowerCase()];
  ...
  if (pendingAction.type === "delete") {
    ...
    if (rowsToEdit.length > 0) {
      // ✅ Send entire array to onEdit for bulk
      onEdit(rowsToEdit, -1);
    }
  }
  ...
  return (
    <div>
      {selectedIds.length > 1 && (
        <ActionMenu
          className="absolute top-0 right-0 mt-5 lg:mt-3 border-foreground/30 mr-4"
      ...
      <div className="overflow-x-auto border border-ring/50 rounded-sm ">
        <table className="min-w-full text-sm">
            ...
            <tr>
              {head.map((h, i) => {
                ...
                if (key === "id") {
                  ...
                  return (
                    <th key={i} className="border-b border-ring/50 px-4 py-2 text-start group">
                        ...
                        <span
                          ...
                          onClick={() => {
                            ...
                            setSortDirection((prev) =>
                              sortColumn === h && prev === "asc" ? "desc" : "asc"
                            );
                          }}
                        >
                          ID
                ...
                if (key === "action") {
                  return (
                    <th key={i} className="border-b text-md font-bold border-ring/50 px-4 py-2 text-start">
                      {h}
                    </th>
                  );
                }
          ...
          <tbody>
            {sortedBody.length === 0 ? (
              <tr>
                <td colSpan={head.length} className="text-center py-6 text-muted-foreground">
                  No record available
                </td>
              </tr>
            ) : (
              sortedBody.map((item, rowIndex) => (
                <tr key={rowIndex} className="border-b border-ring/50 hover:bg-thead-background/40">
                  {head.map((column, colIndex) => {
                        ...
                        <td key={colIndex} className="px-4 py-2">
                          <div className="flex gap-2">
                            ...
                            <ImageButton
                              ...
                              onClick={() => {
                                setPendingAction({ type: "delete", ids: [item.id] });
                                setWarningOpen(true);
                              }}
                            />
                          </div>
                    ...
                    return (
                      ...
                      onClick={() => onCellClick?.(key, String(cellValue))}
                        ...
                        title={`Click to filter ${column} = "${cellValue}"`}
                      >
                        {cellValue}
                      </td>
                    );
